...
namespace AudioConstants {
    constexpr double sqrt2over2 = 0.707106781186547524401; // same as M_SQRT1_2
    
    // constants for dealing with overflow or underflow
    constexpr float FLT_EPSILON_PLUS = 1.192092896e-07f;        /* smallest such that 1.0+FLT_EPSILON != 1.0 */
    constexpr float FLT_EPSILON_MINUS = -1.192092896e-07f;       /* smallest such that 1.0-FLT_EPSILON != 1.0 */
    constexpr float FLT_MIN_PLUS = 1.175494351e-38f;            /* min positive value */
    constexpr float FLT_MIN_MINUS = -1.175494351e-38f;          /* min negative value */
}

// C++17: Use constexpr for compile-time constants
namespace PluginConstants {
    constexpr UINT CURRENT_PLUGIN_API = 61;
    constexpr UINT CONTROL_THEME_SIZE = 32;
    constexpr UINT PLUGIN_CONTROL_THEME_SIZE = 64;
    constexpr UINT CONTROL_THEME = 0;
    constexpr UINT PRESET_COUNT = 16; // more than 16 in VST gets sluggish depending on the client
}
...
// C++17: Use constexpr for compile-time constants
namespace JSProgramConstants {
    constexpr UINT MAX_JS_PROGRAM_STEPS = 16;
    constexpr UINT MAX_JS_PROGRAM_STEP_VARS = 7;
    constexpr UINT JS_PROGRAM_CHANGE = 0xFFFFFFFF;
}
...
{
	int n = (int)strlen(pString1);
	int m = (int)strlen(pString2);
