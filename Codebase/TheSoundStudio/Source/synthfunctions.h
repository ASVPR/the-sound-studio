...
// C++17: Replace macros with constexpr for better type safety
namespace SynthConstants {
    constexpr double CONVEX_LIMIT = 0.00398107;
    constexpr double CONCAVE_LIMIT = 0.99601893;
    constexpr unsigned long ARC4RANDOMMAX = 4294967295UL; // (2^32 - 1)
}

// C++17: Convert macro to constexpr function template
template<typename T>
constexpr T extractBits(T the_val, int bits_start, int bits_len) noexcept {
    return (the_val >> (bits_start - 1)) & ((1 << bits_len) - 1);
}
...
//----------------------------------------------------------------
//
// C++17: Use constexpr for MIDI constants
namespace MidiConstants {
    // CHANNEL VOICE MESSAGES
    constexpr unsigned char NOTE_OFF = 0x80;
    constexpr unsigned char NOTE_ON = 0x90;
    constexpr unsigned char POLY_PRESSURE = 0xA0;
    constexpr unsigned char CONTROL_CHANGE = 0xB0;
    constexpr unsigned char PROGRAM_CHANGE = 0xC0;
    constexpr unsigned char CHANNEL_PRESSURE = 0xD0;
    constexpr unsigned char PITCH_BEND = 0xE0;
...
static inline float fastpow2a (float p)
{
...
{
  return fastpow2a (1.442695040f * p);
}
...
inline double pitchShiftMultiplierOctaves(double numOctaves)
{
    if(numOctaves == 0)
        return 1.0;
...
inline double calcValueVoltOctaveExp(double dLowLimit, double dHighLimit, double dControlValue)
{
	...
	// exp control
	return dLowLimit*fastpow2a(dControlValue*dOctaves);
}
...
//    fNoise = (float)arc4random(); fix Linux
	...
	// normalize and make bipolar
	fNoise = 2.0*(fNoise/SynthConstants::ARC4RANDOMMAX) - 1.0;  
#endif
...
{
	// get the bits
	UINT b0 = extractBits(uPNRegister, 1, 1); // 1 = b0 is FIRST bit from right
	UINT b1 = extractBits(uPNRegister, 2, 1); // 1 = b1 is SECOND bit from right
	UINT b27 = extractBits(uPNRegister, 28, 1); // 28 = b27 is 28th bit from right
	UINT b28 = extractBits(uPNRegister, 29, 1); // 29 = b28 is 29th bit from right
...
const double B2 = 4.0/(float)M_PI;
const double C2 = -4.0/((float)M_PI*(float)M_PI);
const double P = 0.225;
// http://devmaster.net/posts/9648/fast-and-accurate-sine-cosine
// input is -pi to +pi
inline double parabolicSine(double x, bool bHighPrecision = true)
{
    double y = B2 * x + C2 * x * fabs(x);
...
const double D2 = 5.0*(float)M_PI*(float)M_PI;
inline double BhaskaraISine(double x)
{
    double d = fabs(x);
	double sgn = d == 0 ? 1.0 : x/fabs(x);
	return 16.0*x*((float)M_PI - sgn*x)/(D2 - sgn*4.0*x*((float)M_PI - sgn*x));
}
